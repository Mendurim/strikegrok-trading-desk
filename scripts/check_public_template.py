...
def read_preview(url: str, html_path: pathlib.Path | None) -> str:
    ...
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "strikegrok-public-template-check/1"},
    )
...
def main() -> None:
    ...
    if share_url is None:
        # Nothing has been published yet, so there is no preview to compare the
        # manifest against. Say that, rather than reporting a pass for a check
        # that did not run.
        print("public template: skipped - template is ready-to-publish, no Bot published yet")
        return
    try:
        page = read_preview(str(share_url), args.html)

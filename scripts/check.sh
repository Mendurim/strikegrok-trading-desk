...
cli_re = re.compile(r"\bstrikegrok (doctor|market|limits|size|plan-order|execute-order|quickstart|account|defillama|coingecko|order-status)\b|pip install -e|src/strikegrok|strikegrok\.cli")
for path in md_files + glob.glob("rules/*.mdc") + glob.glob("**/*.json", recursive=True):
    ...
    if cli_re.search(text):
        err(f"{path}: references a strikegrok CLI command that does not exist")
...
python3 scripts/test_strike_request.py

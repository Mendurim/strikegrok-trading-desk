...
def main(root):
    ...
    if template.get("name") != "StrikeGrok Desk Lead":
        errors.append(f"{TEMPLATE}: name must be 'StrikeGrok Desk Lead'")
    ...
    elif not all(term in description.lower() for term in ("research-only", "no token", "order")):
    ...
    expected_urls = {
        "bootstrap": f"{REPOSITORY}/blob/{expected_release}/skills/strikegrok-bootstrap/SKILL.md",

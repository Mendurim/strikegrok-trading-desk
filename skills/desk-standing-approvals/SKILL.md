...
- **Tier 1 refuses outright unless `STRIKEGROK_STATE_TRUSTED=1` is set**, which is the operator asserting that the signer really does run as its own user. Without it, keep the platform's Require Approval rule on and trade at Tier 2. `desk_policy.py verify` prints which it is.
...
12. **The reservation is taken at the gate, not at the send.** The gate and the send are two moments, and the gap between them is one network round trip. A layer that recorded the slot after the send would show every Bot inside that window the same free slot - six concurrent processes went through a `max_open` of one that way, which is what `TestConcurrentBots` now pins down. `commit()` afterwards only marks the reservation *sent*.
    ...
    - **A market is released only when it is flat, has nothing resting, *and* one bar has passed.** An entry still working has no position either, and freeing its place at once would let a second order in under a `max_open` of one.

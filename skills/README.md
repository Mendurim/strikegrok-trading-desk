...
Each skill's `metadata.version` is that skill's own, not the release tag. A skill at `1.0.1` inside release `v3.1.2` has simply not needed changing; the release the desk was built from is the tag the bootstrap skill clones.

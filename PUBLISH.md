...
```bash
git add -A && git commit -m "StrikeGrok v3.1.2"
gh repo create "$GH_OWNER/strikegrok-trading-desk" --public --source=. --push
git tag -a v3.1.2 -m "StrikeGrok v3.1.2" && git push --tags
```

The tag matters. The bootstrap skill clones `--branch v3.1.2`, so the desk your
...
> `https://github.com/Mendurim/strikegrok-trading-desk/blob/v3.1.2/skills/strikegrok-bootstrap/SKILL.md`.
> Follow the bootstrap skill, use
> `https://github.com/Mendurim/strikegrok-trading-desk/blob/v3.1.2/SETUP.md` for the

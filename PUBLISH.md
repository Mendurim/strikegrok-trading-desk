...
```bash
git add -A && git commit -m "StrikeGrok v3.0.1"
gh repo create "$GH_OWNER/strikegrok-trading-desk" --public --source=. --push
git tag -a v3.0.1 -m "StrikeGrok v3.0.1" && git push --tags
```

The tag matters. The bootstrap skill clones `--branch v3.0.1`, so the desk your
...
> `https://github.com/Mendurim/strikegrok-trading-desk/blob/v3.0.1/skills/strikegrok-bootstrap/SKILL.md`.
> Follow the bootstrap skill, use
> `https://github.com/Mendurim/strikegrok-trading-desk/blob/v3.0.1/SETUP.md` for the

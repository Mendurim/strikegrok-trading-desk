...
```bash
git add -A && git commit -m "StrikeGrok v3.1.0"
gh repo create "$GH_OWNER/strikegrok-trading-desk" --public --source=. --push
git tag -a v3.1.0 -m "StrikeGrok v3.1.0" && git push --tags
```

The tag matters. The bootstrap skill clones `--branch v3.1.0`, so the desk your
...
> `https://github.com/Mendurim/strikegrok-trading-desk/blob/v3.1.0/skills/strikegrok-bootstrap/SKILL.md`.
> Follow the bootstrap skill, use
> `https://github.com/Mendurim/strikegrok-trading-desk/blob/v3.1.0/SETUP.md` for the
